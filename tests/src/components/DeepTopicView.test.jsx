...
import DeepTopicView from '../../../src/components/DeepTopicView.jsx'
...
vi.mock('../../../src/components/PdfViewer.jsx', () => ({ default: ({ url }) => <div data-testid="pdf">{url}</div> }))
...
test('adds a takeaway to the current section', async () => {
  const { addTakeaway } = await import('../../../src/lib/db/deepTopics.js')
  render(<DeepTopicView supabase={{}} topicId="t1" onBack={vi.fn()} addToast={vi.fn()} />)
...
test('adds a section and advances the cursor to it', async () => {
  const { addSection } = await import('../../../src/lib/db/deepTopics.js')
  render(<DeepTopicView supabase={{}} topicId="t1" onBack={vi.fn()} addToast={vi.fn()} />)
  await screen.findAllByText('Ch.1 Order books')
...
test('renders a PDF source pane for pdf resources', async () => {
  const { getDeepTopic } = await import('../../../src/lib/db/deepTopics.js')
