...
### `showFounderUploads` hasn't moved into the module system
Everything else migrated to the three-layer gate (`src/lib/modules.js`), and
`showFounderFeatures` is deleted. `showFounderUploads` remains because
`NoteEditor.jsx` resolves it with its own `getUser()` call instead of receiving
tier from `App.jsx` — migrating it means threading entitlement down or reading it
locally. Should become the `uploads` module (`minTier: 'founder'`), which already
exists in the registry but is unused. Uploads stay backend-enforced by Storage
RLS regardless, so this is tidiness, not a hole.

### Duplicated `isSafeUrl`
Exists as `supabase/functions/_shared/isSafeUrl.ts` and again pasted inside the
capture function. Two copies of a security check drift, and the pasted one is the
one that won't get the fix.
