...
|---|---|---|
| 1 | **Sort Inbox folds into Tidy.** Same one-card-at-a-time shape; Tidy's actions (move/done/snooze/trash/keep) are a strict superset of Sort Inbox's (assign/delete). Widen the queue to include all inbox items, delete `SortInbox.jsx` (46 lines). Each row already carries a `tidyReason` string, which generalises to `in inbox 2h` with no new concept. **Watch:** the nav inbox badge counts off `inboxEntries` in App.jsx; keep it fed or it goes stale silently. | high |
