...
| ~~4~~ | ~~Write `index_status = 'pending'`~~ | bug | ✅ **DONE 2026-08-24.** `chunkEntryAsync` now claims the entry as `pending` before the first source is processed, so an abandoned import is caught by the retry banner instead of keeping `index_status = null` (which `listUnindexed` does not select). `pending` deliberately does **not** stamp `indexed_at` — that column answers "when did this last complete". Three regression tests, two verified to fail without the fix | `tech-debt.md` |
